...
export const ClientesController = {
    ...
    renderTable(clients) {
        ...
        if (clients.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" class="text-center py-6 text-gray-500">No se encontraron clientes.</td></tr>';
            return;
        }
        ...
        clients.forEach(client => {
            const row = `
                <tr class="hover:bg-gray-50 transition duration-150" data-client-id="${client.id}">
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">${client.razon_social}</td>
