 ...
 * - El botón de Facturación se OCULTA si la OT no está Completada.
 * - Corrección visual de ID (OT vs COT).
 */
...
export const OrdenDetalleController = {
    ...
    setupInvoiceModal() {
        if (document.getElementById('invoice-selection-modal')) return;

        const modalHTML = `
        <div id="invoice-selection-modal" class="fixed inset-0 bg-gray-900 bg-opacity-50 hidden flex items-center justify-center z-50 backdrop-blur-sm">
            <div class="bg-white rounded-xl shadow-2xl p-6 w-96 transform scale-100 transition-transform">
                <div class="text-center mb-6">
                    ...
                    <h3 class="text-lg font-bold text-gray-900">Emitir Documento Fiscal</h3>
                    <p class="text-sm text-gray-500 mt-2">Seleccione el tipo de comprobante a generar para esta Orden de Trabajo.</p>
                </div>
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <button id="btn-modal-boleta" class="flex flex-col items-center justify-center p-4 border-2 border-gray-200 rounded-lg hover:border-green-500 hover:bg-green-50 transition group">
                        <span class="font-bold text-gray-700 group-hover:text-green-700">BOLETA</span>
                        <span class="text-xs text-gray-400 mt-1">Consumidor Final</span>
                    </button>
                    <button id="btn-modal-factura" class="flex flex-col items-center justify-center p-4 border-2 border-gray-200 rounded-lg hover:border-blue-500 hover:bg-blue-50 transition group">
                        <span class="font-bold text-gray-700 group-hover:text-blue-700">FACTURA</span>
                        <span class="text-xs text-gray-400 mt-1">Con RUC</span>
                    </button>
                </div>
                <button id="btn-modal-cancel" class="w-full py-2 text-sm text-gray-500 hover:text-gray-700 font-medium">Cancelar</button>
            </div>
        </div>`;
        document.body.insertAdjacentHTML('beforeend', modalHTML);

        document.getElementById('btn-modal-cancel').onclick = () => document.getElementById('invoice-selection-modal').classList.add('hidden');
        document.getElementById('btn-modal-boleta').onclick = () => this.processInvoiceGeneration('BOLETA');
        document.getElementById('btn-modal-factura').onclick = () => this.processInvoiceGeneration('FACTURA');
    },

    async processInvoiceGeneration(type) {
        document.getElementById('invoice-selection-modal').classList.add('hidden');
        if (!this.currentOrderId) return;
        window.UI.showNotification('Procesando...', 'Generando documento fiscal...');
            ...
            case 'ot': 
                ...
                if (btnInvoice) {
                    if (status === 'Completado') {
                        // --- ESTADO COMPLETADO: MOSTRAR BOTÓN ---
                        btnInvoice.classList.remove('hidden'); // Asegurar que sea visible

                        if (orderData && orderData.estado_facturacion === 'Facturado') {
                            // YA FACTURADO
                            btnInvoice.disabled = true;
                            btnInvoice.innerHTML = `<i data-lucide="check-circle" class="w-4 h-4 mr-2"></i> Facturado`;
                            ...
                            btnInvoice.innerHTML = `<i data-lucide="file-text" class="w-4 h-4 mr-2"></i> Generar Factura`;
                            btnInvoice.classList.add('bg-green-600', 'hover:bg-green-700');
                            btnInvoice.classList.remove('bg-green-400', 'cursor-not-allowed');
                        }
                    } else {
                        // --- NO COMPLETADO: OCULTAR BOTÓN POR COMPLETO ---
    ...
    renderSearchResults(results) {
        ...
        if (results.length === 0) resultsDiv.innerHTML = '<div class="p-3 text-sm text-gray-500 text-center">No se encontraron coincidencias.</div>';
            ...
            results.forEach(client => {
                ...
                div.innerHTML = `<div><div class="font-bold text-gray-800 text-sm group-hover:text-red-600 transition">${client.razon_social}</div><div class="text-xs text-gray-500">RUC: ${client.ruc}</div></div>`;
                div.addEventListener('click', () => { this.setClientSelection(client); resultsDiv.classList.add('hidden'); });
    ...
    setupEvents(currentId) {
        ...
        bindBtn('btn-generate-invoice', () => {
            document.getElementById('invoice-selection-modal').classList.remove('hidden');
        });
    }
};